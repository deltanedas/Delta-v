...
public sealed class DroppableBorgModuleSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<DroppableBorgModuleComponent, ComponentShutdown>(OnShutdown);
    ...
    private void OnMapInit(Entity<DroppableBorgModuleComponent> ent, ref MapInitEvent args)
    {
        ...
        foreach (var slot in ent.Comp.Items)
        {
            ...
            var placeholder = _placeholder.GivePlaceholder(item!.Value, slot.Id, slot.Whitelist);
            ent.Comp.Placeholders.Add(placeholder);
        }

        Dirty(ent);
    }

    private void OnShutdown(Entity<DroppableBorgModuleComponent> ent, ref ComponentShutdown args)
    {
        if (_net.IsClient)
            return;

        foreach (var uid in ent.Comp.Placeholders)
        {
            QueueDel(uid);
        }
    }
    ...
    private void OnModuleSelected(Entity<DroppableBorgModuleComponent> ent, ref BorgModuleSelectedEvent args)
    {
        ...
        for (int i = 0; i < ent.Comp.Items.Count; i++)
        {
            ...
            else
            {
                ...
                if (HasComp<HandPlaceholderComponent>(item)) // prevent dropping it for empty hand gaming
    ...
    public IReadOnlyList<EntityUid> GetItems(Entity<DroppableBorgModuleComponent> ent)
    {
        // mapinit hasn't ran yet
