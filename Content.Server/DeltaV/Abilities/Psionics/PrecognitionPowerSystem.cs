...
public sealed class PrecognitionPowerSystem : EntitySystem
{
    ...
    private void OnShutdown(Entity<PrecognitionPowerComponent> ent, ref ComponentShutdown args)
    {
        _actions.RemoveAction(ent, ent.Comp.PrecognitionActionEntity);
        if (TryComp<PsionicComponent>(ent, out var psionic))
            psionic.ActivePowers.Remove(ent.Comp);
    }

    private void OnPowerUsed(Entity<PrecognitionPowerComponent> ent, ref PrecognitionPowerActionEvent args)
    {
        var ev = new PrecognitionDoAfterEvent();
        var (uid, comp) = ent;
        var doAfterArgs = new DoAfterArgs(EntityManager, uid, comp.UseDelay, ev, uid)
        {
            BreakOnDamage = true
        };
...
`        // A custom shader for seeing visions would be nice but this will do for now.
        _statusEffects.TryAddStatusEffect<TemporaryBlindnessComponent>(uid, "TemporaryBlindness", comp.UseDelay, true);
        _statusEffects.TryAddStatusEffect<SlowedDownComponent>(uid, "SlowedDown", comp.UseDelay, true);

        comp.SoundStream = _audio.PlayEntity(comp.VisionSound, uid, uid)?.Entity;
    ...
    /// </summary>
    private void OnDoAfter(Entity<PrecognitionPowerComponent> ent, ref PrecognitionDoAfterEvent args)
    {
        if (args.Handled)
            return;
        ...
        if (args.Cancelled)
        {
            ...
            comp.SoundStream = _audio.Stop(comp.SoundStream);
            ...
            _actions.SetCooldown(comp.PrecognitionActionEntity, start, start + comp.RetryDelay);
            return;
        }
        ...
        if (!_mind.TryGetMind(uid, out _, out var mind) || mind.Session is not {} session)
            return;
        ...
        if (_random.Prob(comp.RandomResultChance)) // This will replace the proper result message with a random one occasionaly to simulate some unreliablity.
